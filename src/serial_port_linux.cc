...
unsigned get_baud_rate(unsigned long baud) {
  ...
  switch (baud) {
    ...
    case 4000000:
      ...
      break;
    default:
      throw std::invalid_argument("Requested baud rate of " +
...
void serial_port::SerialPortLinux::Open() {
  ...
  // ioctl(handle_, TIOCEXCL);
  // -1 errno

  if (flock(handle_, LOCK_EX | LOCK_NB) < 0) {
    throw IoException(
        "[SerialPortLinux::Open()] Error: could not lock serial port for "
        "exclusive use. " +
        std::string(strerror(errno)));
  }

  if (tcgetattr(handle_, &tty_) != 0) {
    throw IoException("[SerialPortLinux::Open()] Error from tcgetattr(): " +
                      std::string(strerror(errno)));
  }
...
bool serial_port::SerialPortLinux::IsOpen() { return handle_ >= 0; }
...
unsigned long serial_port::SerialPortLinux::ReadData(char* data,
                                                     unsigned long num_bytes) {
  return read(handle_, data, num_bytes);
}

unsigned long serial_port::SerialPortLinux::WriteData(const char* data,
                                                      unsigned long num_bytes) {
